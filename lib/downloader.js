...
async function downloadOne(options) {
  ...
  console.log(panel("Download job", [
    ...
    `${colors.bold}Preset:${colors.reset} ${preset.label}`,
    `${colors.bold}Output:${colors.reset} ${outputDir}`,
    `${colors.dim}${preset.description}${colors.reset}`
  ]));
  ...
  await runYtDlp(args);
  await addHistory({ url: options.url, preset: options.preset || "best", output: outputDir });
}
